...
class WatchdogTask(Task):
    def __init__(self, scheduler, pubsub, wifi, period_ms=1000):
        super().__init__(scheduler, period_ms)
        pubsub.subscribe("watchdog/ack",self.on_ack )
        self.pubsub = pubsub
        self.wifi=wifi
        self.seq = 0
        self.pending = {}   # seq → timestamp
        self.sent=0
        self.recv=0
        self.last_rtt=0
        self.jitter=0
...
#         }
    ...
    def on_ack(msg):
        ...
        if seq in pending:
            ...
            rtt = time.ticks_diff(now, t0)

            # latency
            self.last_rtt= rtt

            # jitter (simple)
            self.jitter = abs(rtt - stats["last_rtt"])
